...
A simple handler that is useful to debug incoming traffic. It will log
entry and exit points of http requests as well as the response code.

== Usage in standard distribution

Simply include jetty-debug.xml in your *.ini configs. For example in
start.ini.
