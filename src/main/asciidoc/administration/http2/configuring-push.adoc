...
This allows the `PushCacheFilter` to organize resources in _primary_ resources (those referenced by the `Referer` header) and _secondary_ resources (those that have the `Referer` header).
